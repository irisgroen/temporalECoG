...
stimData = load('/Volumes/server/Projects/BAIR/Data/BIDS/visual/stimuli/sub-umcuchaam_ses-UMCUECOGday03_task-bairspatialpattern_run-2_acq-clinical_events.mat');
...
% PLOT contrast level versus MAX
figure('Name', [condName ' contrast response functions']);hold on
colors = out.colors;

for ee = 1:length(elecNames)
    subplot(specs.plot.nSubPlots(1),specs.plot.nSubPlots(2),ee);hold on
    for ii = 1:length(whichTrials)
        plot(CRFrms(ii),maxResp(ii,ee),'Color', colors(ii,:), 'Marker', '.', 'MarkerSize', 50,'LineStyle', 'none');
...
figure('Name', [condName ' max response latency']);hold on

for ee = 1:length(elecNames)
    subplot(specs.plot.nSubPlots(1),specs.plot.nSubPlots(2),ee);hold on
    for ii = 1:length(whichTrials)
        plot(CRFrms(ii),maxTmp(ii,ee),'Color', colors(ii,:), 'Marker', '.', 'MarkerSize', 50,'LineStyle', 'none');
    end
	%xlim([0 length(whichTrials)+1]);
...
% PLOT contrast level versus SUM
figure('Name', [condName ' contrast versus sum']);hold on
colors = out.colors;

for ee = 1:length(elecNames)
    subplot(specs.plot.nSubPlots(1),specs.plot.nSubPlots(2),ee);hold on
    for ii = 1:length(whichTrials)
        plot(CRFrms(ii),sumResp(ii,ee),'Color', colors(ii,:), 'Marker', '.', 'MarkerSize', 50,'LineStyle', 'none');
        %e = errorbar(CRFrms(ii),maxResp(ii,ee),maxRespSE(ii,ee), 'LineWidth', 2,'LineStyle', 'none', 'Color', colors(ii,:));
        %e.Annotation.LegendInformation.IconDisplayStyle = 'off';
    end
	%xlim([0 length(whichTrials)+1]);
...
figure('Name', [condName ' contrast response functions electrode-average']);hold on

for ee = 1:length(eeToAverage)
    elIndex = eeToAverage{ee};
    maxToPlot = mean(maxResp(:,elIndex),2);
    maxToPlotSE = std(maxResp(:,elIndex),0,2)/length(elIndex);
    subplot(1,3,ee); hold on;
    for ii = 1:length(whichTrials)
        plot(CRFrms(ii),maxToPlot(ii),'Color', colors(ii,:), 'Marker', '.', 'MarkerSize', 50,'LineStyle', 'none');
        e = errorbar(CRFrms(ii),maxToPlot(ii),maxToPlotSE(ii), 'LineWidth', 2,'LineStyle', 'none', 'Color', colors(ii,:));
        e.Annotation.LegendInformation.IconDisplayStyle = 'off';
    end
	%xlim([0 length(whichTrials)+1]);
    ylim([0 max(maxResp(:)) + 0.1*max(maxResp(:))]);
...
figure('Name', [condName ' max response latency electrode-average']);hold on

for ee = 1:length(eeToAverage)
    elIndex = eeToAverage{ee};
    maxToPlot = mean(maxTmp(:,elIndex),2);
    maxToPlotSE = std(maxTmp(:,elIndex),0,2)/length(elIndex);
    subplot(1,3,ee); hold on;
    for ii = 1:length(whichTrials)
        plot(CRFrms(ii),maxToPlot(ii),'Color', colors(ii,:), 'Marker', '.', 'MarkerSize', 50,'LineStyle', 'none');
        e = errorbar(CRFrms(ii),maxToPlot(ii),maxToPlotSE(ii), 'LineWidth', 2,'LineStyle', 'none', 'Color', colors(ii,:));
...
for ee = 1:length(eeToAverage)
    ...
    maxToPlotSE = std(sumResp(:,elIndex),0,2)/length(elIndex);
    subplot(1,3,ee); hold on;
    for ii = 1:length(whichTrials)
        plot(CRFrms(ii),maxToPlot(ii),'Color', colors(ii,:), 'Marker', '.', 'MarkerSize', 50,'LineStyle', 'none');
        e = errorbar(CRFrms(ii),maxToPlot(ii),maxToPlotSE(ii), 'LineWidth', 2,'LineStyle', 'none', 'Color', colors(ii,:));
